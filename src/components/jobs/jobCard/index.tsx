...
import { ButtonVariant, Job } from "../jobsTypes";
...
export default function JobCard({ job }: Props) {
  ...
  const formatSalary = (amount: string | number | undefined): string => {
    if (!amount) return "0";
    const numericAmount = parseInt(String(amount), 10);
    return numericAmount >= 1000 ? `${numericAmount / 1000}k` : String(amount);
  };
  ...
  const jobId =
    typeof job._id === "object" && job._id !== null && "$oid" in job._id
      ? job._id.$oid
      : (job._id as string);
